...
def calibrate(ps, treatment):
    ...
    # Reshape ps to a 2D array for LogisticGAM.fit
    ps_2d = np.reshape(ps, (-1, 1))
    gam = LogisticGAM(s(0)).fit(ps_2d, treatment)

    # Also reshape ps for prediction
    return gam.predict_proba(ps_2d)
